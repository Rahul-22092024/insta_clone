...
const localStrategy = require("passport-local");// es line se username aur password ko authenticate kiya jaega aur jab agli baar wo login karega to wo username aur password ko jaruye hhi login kr payega
const userModel = require("./users");
const postModel = require("./posts");
const storyModel = require("./story");
passport.use(new localStrategy(userModel.authenticate()));   // logged in rhne k liye h 
...
router.post("/update", isLoggedIn, upload.single('image'), async function (req, res) {
  const user = await userModel.findOneAndUpdate(  //esse wo bnda mil jayega jo lohin hua h
    { username: req.session.passport.user },
...
router.post(
  "/upload",
  isLoggedIn,
  upload.single("image"), // upload ho chuka h 
  async function (req, res) { // ab upload honee k baad ka kaam
    const user = await userModel.findOne({
      username: req.session.passport.user,
    });
     const post = await postModel.create({
...
router.post("/register", function (req, res) {
  const user = new userModel({
    username: req.body.username, //index page pr jo input m likha h wo , mtlb register krte smy jo bhi username m data aaye usko username m save kr dena
    email: req.body.email,
    name: req.body.name,
  });

  userModel.register(user, req.body.password)/* pichhe wale se promise return hoga jisko ham .then se lenge*/.then(function (registereduser) {  
    passport.authenticate("local")(req, res, function () {// etne se register hua h ab  bnda login  rhe uska code dekhneg
      res.redirect("/profile");  // login ho gya
    });
  });
});
...
router.post(
  "/login",
  passport.authenticate("local",/* local se mtlb username aur passport k bases pr login kro */ {
